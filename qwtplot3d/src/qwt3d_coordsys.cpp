...
void CoordinateSystem::init(Triple first, Triple second)
{
  ...
  for (unsigned i=0; i!=axes.size(); ++i)
    ...
    double majlx =  dv.y / 20, minlx = majlx * 0.6;
    double majly =  dv.x / 20, minly = majly * 0.6;
    double majlz =  dv.x / 20, minlz = majlz * 0.6;

    axes[X1].setTicLength(majlx, minlx);
    axes[X2].setTicLength(majlx, minlx);
    axes[X3].setTicLength(majlx, minlx);
    axes[X4].setTicLength(majlx, minlx);

    axes[Y1].setTicLength(majly, minly);
    axes[Y2].setTicLength(majly, minly);
    axes[Y3].setTicLength(majly, minly);
    axes[Y4].setTicLength(majly, minly);

    axes[Z1].setTicLength(majlz, minlz);
    axes[Z2].setTicLength(majlz, minlz);
    axes[Z3].setTicLength(majlz, minlz);
    axes[Z4].setTicLength(majlz, minlz);
