C      PROGRAM SINGWI_SJOLANDER_BAYES_POLY
c
      SUBROUTINE ssfit(nd,X_in,Y_in,E_in,sfile,l_fn,
     1 kill,res,no,XOUT,YOUT)
      INCLUDE 'jump_inc.f90'
      integer nd, l_fn
cf2py intent(in) :: nd, l_fn                    !#points & length filename
      real X_in(mpt), Y_in(mpt), E_in(mpt)
...
cf2py intent(out) :: res, YOUT, XOUT           !TOTALS out
      REAL    XXX(mpt,mpar),SIG2(mpt)
      REAL    XM(mpar),YLOGPM(mpar),AM(mpar,mpar),SIGAM(mpar,mpar)
      REAL    BIGHES(mpar,mpar),BIGAMX(mpar,mpar)
      REAL    GRAD(mpar),HESS(mpar,mpar),COVAR(mpar,mpar)
      REAL    AMCHD(mpar)
      INTEGER INDX(mpar+2)
...
C
      ...
      CALL DATINT(SIG2)
      WRITE(6,1001)NDAT
1001  format(' ssfit> No. of data read in = ',i5)
      close(unit=6)
...
1102    format(' ssfit> *** Dodgy refinement :',2e13.4)
        close(unit=6)
        STOP
 ...
 141  FORMAT(' Log10[Prob(Singwi-Sjolander|{Data})] = ',F10.1)
      WRITE(6,131) AM(1,1),SIGAM(1,1)
 131  FORMAT(' Coeff. A =  ',1PE10.3,' +- ',E9.2)
      WRITE(6,132) AM(2,1)*2.0,SIGAM(2,1)*2.0
 132  FORMAT(' Coeff. RR =  ',1PE10.3,' +- ',E9.2)
      CLOSE(UNIT=6)
      res(1)=CHISQ
      res(2)=YLOGPM(1)
      res(3)=AM(1,1)
...
C
     ...
     1  access='append')
       write(6,1000)
1000   format(' ssfit> something wrong here!')
       close(unit=6)
       STOP
      endif
      CMIN=1.0E+20
...
1102    format(' ssfit> Minimisation problem!! :',2e13.4)
       close(unit=6)
       STOP
      endif
      SUM1=0.0
